...
async function prepareItems(adapter) {
	...
	adapter.setState('info.diagnostics.itemsTotal', validItems.length, true);
	adapter.setState('info.itemsActive', enabledItems.length, true);

	await Promise.all(validItems.map(item => stateRegistry.ensureOutputState(adapter, item)));
	...
	await Promise.all(
		Array.from(adapter.compiledItems.values()).map(async c => {
			try {
				await stateRegistry.ensureItemInfoStatesForCompiled(adapter, c);
				const base = stateRegistry.getItemInfoBaseId(c.outputId);
				adapter.setState(`${base}.compiledOk`, !!c.ok, true);
				adapter.setState(`${base}.compileError`, c.ok ? '' : String(c.error || 'compile failed'), true);
			} catch (e) {
	...
	if (enabledItems.length === 0) {
		...
		adapter.setState('info.status', 'no_items_enabled', true);
	} else {
		adapter.setState('info.status', 'ok', true);
	}
}
