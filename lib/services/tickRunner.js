...
async function runTick(adapter) {
	...
	for (let idx = 0; idx < enabledItems.length; idx++) {
		...
		try {
			...
			adapter.setState(targetId, value, true);
			...
			if (adapter.lastGoodValue.has(targetId) && next <= retriesBeforeZero) {
				try {
					adapter.setState(targetId, adapter.lastGoodValue.get(targetId), true);
				} catch {
					// ignore write errors
				}
			} else if (next > retriesBeforeZero) {
				try {
					adapter.setState(targetId, evaluator.getZeroValueForItem(item), true);
				} catch {
					// ignore write errors
				}
			}
	...
	// Publish timing diagnostics AFTER eval loop so they don't consume the eval budget.
	try {
		const ids = snapshot && typeof snapshot.keys === 'function' ? Array.from(snapshot.keys()) : [];
		const now = Date.now();
		const sleepThresholdMs = 30000;
