...
// @ts-nocheck
//
// Master/Detail editor for the "items" (values) list of this adapter's jsonConfig.
// Ported from the legacy hand-rolled Module Federation container (admin/custom/customComponents.js,
// GUI API generation 1) to a proper Vite + @module-federation/vite build (generation 2) for Admin 8.
// The render logic below is intentionally kept close to the original createElement-based
// implementation - no rewrite to class-based ConfigGeneric/MUI was necessary, since Admin 8 keeps
// the { data, attr, onChange, socket, theme, themeType, t } props interface for custom components
// (see ConfigCustom.tsx in ioBroker/json-config).
import React from 'react';

const UI_VERSION = '2026-09-04 v0.5.0 (admin-8-guiapi2)';
const DEBUG = false;
...
// AdapterReact (2nd factory arg) used to provide @iobroker/adapter-react-v5's DialogSelectID for a
// "pick an existing state" button. That dependency is intentionally dropped for the Admin 8 rebuild
// (see README note in this folder / CHANGELOG) to avoid pulling in @iobroker/gui-components (and its
// ~10k MUI icon modules) as a Module Federation shared singleton, which blew up local build times
// from seconds to 18+ minutes. All call sites already null-guard AdapterReact, so the "select" buttons
// simply render disabled and users type the state ID manually - no functional loss beyond that dialog.
const DataSolectrusItemsEditor = createDataSolectrusItemsEditor(React, null);

export default DataSolectrusItemsEditor;
